...
export function DrivePage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      other: ((total - docs - sheets) / total) * 100
    };
  }, [files]);
  const handleSummarize = (file: any) => {
    const context = `Analyze the document shard: "${file.name}". Suggest key insights and potential actions.`;
    navigate(`/?context=${encodeURIComponent(context)}`);
  };
  const handleNewShard = () => {
    const context = `Initialize a new document shard. Purpose: [Describe document]`;
    navigate(`/?context=${encodeURIComponent(context)}`);
  };
  return (
    <AppLayout peripheral={
      <div className="p-6 space-y-8 h-full flex flex-col no-scrollbar">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-widest text-white/40">
              <span>Logical Grids</span>
              <span className="text-memory-violet">{Math.round(stats.sheets)}%</span>
            </div>
            <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
              <div className="h-full bg-memory-violet" style={{ width: `${stats.sheets}%` }} />
            </div>
          </div>
        </div>
        <div className="mt-auto">
          <Button onClick={handleNewShard} className="w-full bg-bio-cyan/10 border border-bio-cyan/20 text-bio-cyan hover:bg-bio-cyan/20 font-black uppercase tracking-widest text-[9px] h-12 rounded-xl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 lg:py-12">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((file) => (
              <NeuralCard key={file.id} className="p-6 hover:border-bio-cyan/40 transition-all group relative overflow-hidden flex flex-col h-full">
                <div className="flex items-start justify-between mb-6">
                  <div className="p-3 rounded-2xl bg-white/[0.03] text-bio-cyan group-hover:scale-110 transition-transform">
                    <FileText size={24} />
                  </div>
                  <div className="flex gap-2">
                    <Button onClick={() => handleSummarize(file)} size="icon" variant="ghost" className="h-8 w-8 text-bio-cyan/40 hover:text-bio-cyan hover:bg-bio-cyan/10">
                      <Sparkles size={16} />
                    </Button>
