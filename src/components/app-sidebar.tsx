...
import { Zap, MessageSquare, Database, BrainCircuit, Clock, Settings, Trash2, ExternalLink, CheckCircle2, FileText, Map } from "lucide-react";
    ...
    <Sidebar className="border-r-0 bg-transparent">
      <SidebarHeader className="p-6">
        <Link to="/" className="flex items-center gap-3 group">
          ...
          <div><h1 className="font-black text-lg tracking-tighter text-foreground">C.A.N.S.</h1><p className="text-[9px] uppercase tracking-widest text-bio-cyan/60 font-black">Neural OS v1.2</p></div>
        </Link>
      </SidebarHeader>
      <SidebarContent className="px-4">
                ...
                <SidebarMenuItem key={item.label}>
                  <SidebarMenuButton asChild isActive={active} className={cn("group h-11 rounded-xl px-4 mb-1 transition-all", active ? "bg-bio-cyan/10 text-bio-cyan border border-bio-cyan/20" : "text-white/40 hover:bg-white/5")}>
                    <Link to={item.path} className="flex items-center justify-between w-full">
                      ...
                      {hasLiveNode && <div className="h-1.5 w-1.5 rounded-full bg-[#10b981] animate-pulse shadow-[0_0_5px_#10b981]" />}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              );
        ...
        <SidebarGroup className="mt-8">
          ...
          <div className="px-2 mb-2">
            <Button onClick={async () => { const url = await chatService.getAuthUrl('gmail'); if(url) window.open(url, 'CANS_AUTH', 'width=600,height=700'); }} variant="outline" className={cn("w-full justify-start gap-3 h-11 text-[10px] font-black uppercase tracking-widest", isConnected('gmail') ? "bg-[#10b981]/10 border-[#10b981]/30 text-[#10b981]" : "bg-bio-cyan/5 border-bio-cyan/20 text-bio-cyan")}>
              {isConnected('gmail') ? <CheckCircle2 size={14} /> : <ExternalLink size={14} />}
